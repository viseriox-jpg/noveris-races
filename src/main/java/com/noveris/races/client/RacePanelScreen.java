...
public final class RacePanelScreen extends NoverisScreen {
    ...
    private void renderPowers(GuiGraphics g, Race race) {
        ...
        if (race == Race.HUMAN) {
            ...
            if (race == Race.NPC) {
                g.drawString(font, "[" + ClientEvents.MOBILITY.getTranslatedKeyMessage().getString() + "]  ATIVA COM POUCA VIDA",
                        x, y + 124, MUTED, false);
            } else {
    ...
    private String mobility(Race r){return switch(r){case ELF->"RECUO ACROBÁTICO";case FAIRY->switch(ClientRaceState.fairyAffinity){case WATER->"SALTO DE MARÉ";case AIR->"SALTO ATMOSFÉRICO";default->"SALTO DE CIPÓ";};case SATYR->"INVESTIDA CAPRINA";case THALASSIAN->"IMPULSO AQUÁTICO";case NEPHILIM->"IMPULSO RADIANTE";case VAMPIRE->"PASSO SOMBRIO";case TIEFLING->"AVANÇO EM FOGO";case LYCANTHROPE->"BOTE PREDATÓRIO";case DRAGONBORN->"INVESTIDA DRACÔNICA";case HARPY->"IMPULSO ALADO";case GOD->"TELEPORTE CELESTIAL";case NPC->"CURA DE EMERGÊNCIA";default->"—";};}
