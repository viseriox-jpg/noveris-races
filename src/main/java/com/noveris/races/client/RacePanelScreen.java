...
public final class RacePanelScreen extends NoverisScreen {
    ...
    private void renderOverview(GuiGraphics g, Race race) {
        ...
        if (race == Race.THALASSIAN || race == Race.HALF_BLOOD
                && (ClientRaceState.ancestryA == Race.THALASSIAN || ClientRaceState.ancestryB == Race.THALASSIAN)) {
    ...
    private String active(Race r){return switch(r){case ELF->"DISPARO PERFURANTE";case FAIRY->switch(ClientRaceState.fairyAffinity){case WATER->"VÉU DE MARÉ";case AIR->"RAJADA FEÉRICA";default->"RAÍZES PRENSORAS";};case SATYR->"VIGOR SILVESTRE";case THALASSIAN->"GUARDA DAS MARÉS";case NEPHILIM->"ÉGIDE SOBRENATURAL";case VAMPIRE->"DRENAGEM DE SANGUE";case HALF_BLOOD->"HERANÇA COMBINADA";case TIEFLING->"PULSO INFERNAL";case LYCANTHROPE->"UIVO DE CAÇADA";case DRAGONBORN->"SOPRO ELEMENTAL";case HARPY->"RAJADA DE VENTO";default->"—";};}
    private String mobility(Race r){return switch(r){case ELF->"RECUO ACROBÁTICO";case FAIRY->switch(ClientRaceState.fairyAffinity){case WATER->"SALTO DE MARÉ";case AIR->"SALTO ATMOSFÉRICO";default->"SALTO DE CIPÓ";};case SATYR->"INVESTIDA CAPRINA";case THALASSIAN->"IMPULSO AQUÁTICO";case NEPHILIM->"IMPULSO RADIANTE";case VAMPIRE->"PASSO SOMBRIO";case HALF_BLOOD->"MOBILIDADE HERDADA";case TIEFLING->"AVANÇO EM FOGO";case LYCANTHROPE->"BOTE PREDATÓRIO";case DRAGONBORN->"INVESTIDA DRACÔNICA";case HARPY->"IMPULSO ALADO";default->"—";};}
    private String activeInfo(Race r){return switch(r){case ELF->"Projétil mágico perfurante: 2,5 corações em linha.";case FAIRY->switch(ClientRaceState.fairyAffinity){case WATER->"Empurra inimigos, apaga fogo, remove 1 efeito e protege.";case AIR->"Rajada frontal que empurra entidades sem causar dano.";default->"Prende inimigos próximos com Lentidão em uma área curta.";};case SATYR->"Concede velocidade e regeneração temporárias.";case THALASSIAN->"Cria uma guarda mais forte quando submerso.";case NEPHILIM->"Remove 1 efeito; protege 10s e custa 2 de fome.";case VAMPIRE->"Fere e rouba até 3 corações durante a noite.";case HALF_BLOOD->"Ativa um bônus reduzido de cada ascendência escolhida.";case TIEFLING->"Queima e fere criaturas ao redor.";case LYCANTHROPE->"Revela presas; recebe mobilidade extra à noite.";case DRAGONBORN->"Dispara o elemento da linhagem em uma área frontal.";case HARPY->"Empurra criaturas com uma rajada frontal.";default->"";};}
    private String mobilityInfo(Race r){return switch(r){case ELF->"Salto para trás; remove Lentidão e protege por 0,5s.";case FAIRY->switch(ClientRaceState.fairyAffinity){case WATER->"Impulso curto, mais forte dentro ou ao lado da água.";case AIR->"Impulso vertical apenas no chão, sem voo contínuo.";default->"Impulso sobre chão natural, com salto curto.";};case SATYR->"Investida terrestre veloz com salto curto.";case THALASSIAN->"Arrancada livre na água; em terra exige contato com o chão.";case NEPHILIM->"Impulso radiante que exige contato com o chão.";case VAMPIRE->"Avanço sombrio terrestre na direção do olhar.";case HALF_BLOOD->"Impulso terrestre calculado pelas duas ascendências.";case TIEFLING->"Avanço terrestre envolto em fogo.";case LYCANTHROPE->"Bote terrestre longo para alcançar uma presa.";case DRAGONBORN->"Investida terrestre pesada e predominantemente horizontal.";case HARPY->"Impulso vertical único; não permite voo contínuo.";default->"";};}
    private String[] passives(Race r){return switch(r){case ELF->new String[]{"Visão controlável em baixa luz","Velocidade adicional dentro de florestas"};case FAIRY->new String[]{"Natureza: purifica veneno em terreno natural","Água: resistência quando molhado; Ar: queda lenta ao cair"};case SATYR->new String[]{"Mais rápido e ágil sobre terreno natural","Metade do dano de queda; cura em florestas"};case THALASSIAN->new String[]{"Respira, nada e minera normalmente na água","Resistência física enquanto estiver submerso"};case HUMAN->new String[]{"Pressa leve permanente","Não possui poderes ou resistências raciais"};case NEPHILIM->new String[]{"50% menos dano de fogo; chamas duram menos","Força temporária quando está com pouca vida"};case VAMPIRE->new String[]{"Visão controlável e roubo de vida corpo a corpo","Força e velocidade adicionais durante a noite"};case HALF_BLOOD->new String[]{"Uma característica reduzida de cada ascendência","Ativa e mobilidade combinam as duas origens"};case TIEFLING->new String[]{"Imune a fogo e lava; visão controlável","Recebe poder temporário ao tocar fogo"};case LYCANTHROPE->new String[]{"Força, velocidade e regeneração à noite","Detecta criaturas em um raio curto de 6 blocos"};case DRAGONBORN->new String[]{"12% de resistência física e a empurrão","Resiste ao elemento de sua linhagem"};case HARPY->new String[]{"Salto alto, planagem e queda reduzida","Mais móvel em céu aberto e lugares elevados"};default->new String[0];};}
    private String[] conditions(Race r){return switch(r){case ELF->new String[]{"Afinidade florestal","Visão leve em baixa luz"};case FAIRY->new String[]{"Natureza fortalece regeneração","Nenhuma habilidade de voo"};case SATYR->new String[]{"Terreno natural favorece mobilidade","Armadura pesada limita bônus"};case THALASSIAN->new String[]{minecraft.player!=null&&minecraft.player.isInWater()?"Hidratado e fortalecido":"Desidratação em progresso","Respiração aquática permanente"};case HUMAN->new String[]{"Pressa leve ativa","Sem habilidades nas teclas raciais"};case NEPHILIM->new String[]{"Resistência parcial ativa","Nenhuma habilidade de voo"};case VAMPIRE->new String[]{minecraft.level!=null&&minecraft.level.isNight()?"Poder noturno ativo":"Bônus noturnos suspensos","Roubo de vida corpo a corpo"};case HALF_BLOOD->new String[]{ClientRaceState.ancestryA.title+" + "+ClientRaceState.ancestryB.title,"Heranças reduzidas"};case TIEFLING->new String[]{"Imunidade a fogo ativa","Visão infernal depende da luz","Cura natural reduzida"};case LYCANTHROPE->new String[]{minecraft.level!=null&&minecraft.level.isNight()?"Poder lunar ativo":"Forma diurna enfraquecida","Combate: "+(ClientRaceState.combat?"ATIVO":"livre"),"Fome atual limita os bônus"};case DRAGONBORN->new String[]{"Elemento: "+ClientRaceState.lineage.title,"Resistência física ativa","Metabolismo aumentado"};case HARPY->new String[]{minecraft.player!=null&&minecraft.player.level().canSeeSky(minecraft.player.blockPosition())?"Céu aberto":"Ambiente fechado","Planagem sem ganho de altitude","Peso da armadura afeta asas"};default->new String[]{"Seleção obrigatória"};};}
    private String[] weaknesses(Race r){return switch(r){case ELF->new String[]{"Golpes corpo a corpo fortes causam Lentidão","Resistência física menor"};case FAIRY->new String[]{"No Nether, a regeneração é bloqueada","Lava bloqueia habilidades"};case SATYR->new String[]{"Armadura pesada reduz bônus por etapas","Ambientes subterrâneos reduzem velocidade"};case THALASSIAN->new String[]{"Desidrata ao permanecer longe da água","Fogo aplica Fraqueza temporária"};case HUMAN->new String[]{"Sem resistências naturais especiais","Sem habilidades ativa ou de mobilidade"};case NEPHILIM->new String[]{"Toda cura recebida é 20% menor","Égide termina com Fraqueza I por 3 segundos"};case VAMPIRE->new String[]{"Sol suspende poderes noturnos e roubo de vida","Fogo aplica Fraqueza temporária; comida rende menos"};case HALF_BLOOD->new String[]{"Poderes herdados são versões reduzidas","Também herda parte das fraquezas escolhidas"};case TIEFLING->new String[]{"Regeneração natural é 25% menor","Água aplica Fraqueza temporária"};case LYCANTHROPE->new String[]{"Velocidade e regeneração exigem 6 de fome","Durante o dia, os bônus noturnos ficam suspensos"};case DRAGONBORN->new String[]{"Movimento lento e fome aumentada","Elemento oposto aplica penalidade temporária"};case HARPY->new String[]{"Golpes físicos causam Lentidão breve","Cavernas e armadura pesada limitam as asas"};default->new String[0];};}
}
