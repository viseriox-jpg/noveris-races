...
public final class RaceSelectionScreen extends NoverisScreen {
    ...
    private void renderDetails(GuiGraphics g,int mx,int my){
        ...
        if(selected==Race.DRAGONBORN){
            ...
            else g.drawString(font,"Clique para alternar",specialX+specialW*2+18,specialY+7,MUTED,false);
        }
    }
    ...
    private String active(Race r){return switch(r){case ELF->"Disparo Perfurante";case FAIRY->switch(fairyAffinity){case WATER->"Véu de Maré";case AIR->"Rajada Feérica";default->"Raízes Prensoras";};case SATYR->"Vigor Silvestre";case THALASSIAN->"Guarda das Marés";case HUMAN->"Nenhuma habilidade ativa";case NEPHILIM->"Égide Sobrenatural";case VAMPIRE->"Drenagem de Sangue";case HALF_BLOOD->"Herança Combinada";case TIEFLING->"Pulso Infernal";case LYCANTHROPE->"Uivo de Caçada";case DRAGONBORN->"Sopro Elemental";case HARPY->"Rajada de Vento";default->"—";};}
    private String mobility(Race r){return switch(r){case ELF->"Recuo Acrobático";case FAIRY->switch(fairyAffinity){case WATER->"Salto de Maré";case AIR->"Salto Atmosférico";default->"Salto de Cipó";};case SATYR->"Investida Caprina";case THALASSIAN->"Impulso Aquático";case HUMAN->"Nenhuma habilidade de mobilidade";case NEPHILIM->"Impulso Radiante";case VAMPIRE->"Passo Sombrio";case HALF_BLOOD->"Mobilidade Herdada reduzida";case TIEFLING->"Avanço em Fogo";case LYCANTHROPE->"Bote Predatório";case DRAGONBORN->"Investida Dracônica";case HARPY->"Impulso Alado";default->"—";};}
    private String[] passives(Race r){return switch(r){case ELF->new String[]{"Visão leve em ambientes escuros","Velocidade adicional em florestas"};case FAIRY->new String[]{"Natureza: purifica veneno em terreno natural","Água: resistência breve quando molhado; Ar: queda lenta ao cair"};case SATYR->new String[]{"Rápido no natural; salto alto","Menos queda e regeneração florestal"};case THALASSIAN->new String[]{"Respiração, visão e mineração aquáticas","Nado rápido; resistência submersa"};case HUMAN->new String[]{"Pressa leve permanente","Sem poderes sobrenaturais"};case NEPHILIM->new String[]{"50% menos dano de fogo; Égide purifica","Força temporária com pouca vida"};case VAMPIRE->new String[]{"Visão noturna e roubo de vida","Força e velocidade à noite"};case HALF_BLOOD->new String[]{"Bônus menor de cada origem","Ativa e mobilidade dependem das ascendências"};case TIEFLING->new String[]{"Imunidade a fogo e lava","Visão escura e retaliação"};case LYCANTHROPE->new String[]{"Força e regeneração à noite","Faro para criaturas"};case DRAGONBORN->new String[]{"12% de resistência física","Resistência elemental e a empurrão"};case HARPY->new String[]{"Salto, planagem e queda reduzida","Mobilidade a céu aberto"};default->new String[0];};}
    private String[] weaknesses(Race r){return switch(r){case ELF->new String[]{"Golpes corpo a corpo fortes causam Lentidão","Resistência física menor"};case FAIRY->new String[]{"No Nether, a regeneração é bloqueada","Lava bloqueia habilidades"};case SATYR->new String[]{"Armadura pesada reduz mobilidade","Penalidade subterrânea/artificial"};case THALASSIAN->new String[]{"Desidratação longe da água","Fogo aplica Fraqueza temporária"};case HUMAN->new String[]{"Sem resistências sobrenaturais"};case NEPHILIM->new String[]{"Cura 20% menor","Égide custa fome e termina em Fraqueza"};case VAMPIRE->new String[]{"Sol suspende poderes noturnos; comida rende menos","Fogo aplica Fraqueza temporária"};case HALF_BLOOD->new String[]{"Heranças são reduzidas","Fraquezas herdadas também são reduzidas"};case TIEFLING->new String[]{"Cura natural reduzida","Água causa Fraqueza"};case LYCANTHROPE->new String[]{"Fome limita bônus e habilidades noturnas","Prata aplica Fraqueza temporária"};case DRAGONBORN->new String[]{"Movimento lento e fome aumentada","Elemento oposto aplica penalidade temporária"};case HARPY->new String[]{"Golpes físicos causam Lentidão breve","Cavernas e armadura pesada limitam as asas"};default->new String[0];};}
}
