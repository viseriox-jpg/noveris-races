...
public final class RaceEvents {
    ...
    public static void damage(LivingIncomingDamageEvent event) {
        ...
        if (event.getSource().getEntity() instanceof ServerPlayer attacker
            ...
            // Cura fixa de 0,5 coração; o intervalo de 10 segundos impede
            // que ataques rápidos transformem o Vampiro em uma fonte de cura.
            attacker.heal(1f);
            RaceState.customLong(attacker, "VampireBiteReady", attacker.level().getGameTime() + 200);
        }
        ...
        if (race == Race.FAIRY && event.getSource().is(DamageTypeTags.IS_FALL)
            ...
            event.setAmount(event.getAmount() * RaceConfig.fairyFallDamageMultiplier.get().floatValue());
        ...
        if (race == Race.FAIRY && event.getSource().is(DamageTypeTags.WITCH_RESISTANT_TO))
            event.setAmount(event.getAmount() * RaceConfig.fairyMagicDamageMultiplier.get().floatValue());
        ...
        if (race == Race.DRAGONBORN && !event.getSource().is(DamageTypeTags.BYPASSES_ARMOR))
            event.setAmount(event.getAmount() * RaceConfig.dragonPhysicalDamageMultiplier.get().floatValue());
        if (race == Race.DRAGONBORN) {
            DragonLineage lineage = RaceState.lineage(victim);
            if (lineage == DragonLineage.FIRE && event.getSource().is(DamageTypeTags.IS_FIRE)) event.setAmount(event.getAmount() * RaceConfig.dragonFireDamageMultiplier.get().floatValue());
            ...
            if (lineage == DragonLineage.FROST && event.getSource().is(DamageTypeTags.IS_FIRE))
                victim.addEffect(new MobEffectInstance(MobEffects.WEAKNESS, 80, 0, false, false));
            if (lineage == DragonLineage.FROST && event.getSource().is(DamageTypeTags.IS_FREEZING)) event.setAmount(event.getAmount() * RaceConfig.dragonFrostDamageMultiplier.get().floatValue());
            if (lineage == DragonLineage.VENOM && event.getSource().is(DamageTypeTags.WITCH_RESISTANT_TO))
                victim.addEffect(new MobEffectInstance(MobEffects.WEAKNESS, 80, 0, false, false));
        }
        if (race == Race.HARPY) {
            if (event.getSource().is(DamageTypeTags.IS_FALL)) event.setAmount(event.getAmount() * RaceConfig.harpyFallDamageMultiplier.get().floatValue());
    ...
    public static void fairyPlantFood(LivingEntityUseItemEvent.Finish event) {
        ...
        if (fairy && isPlantFood(event.getItem())) {
    ...
    private static void applyPassives(ServerPlayer p, Race race) {
        switch (race) {
            ...
            case GOD -> {
                p.addEffect(new MobEffectInstance(MobEffects.REGENERATION, 60, 6, false, false));
    ...
    private static void tickHydration(ServerPlayer p, boolean hybrid) {
        ...
        if (dry >= maximum) {
            ...
            if (p.tickCount % RaceConfig.hydrationDamageIntervalTicks.get() == 0 && p.getHealth() > 2f) {
                p.hurt(p.damageSources().dryOut(), Math.min(RaceConfig.hydrationDamage.get().floatValue(), p.getHealth() - 2f));
                if (p.level() instanceof ServerLevel level)
    ...
    private static void handleLycanTransformation(ServerPlayer p, Race race) {
        boolean affected = race == Race.LYCANTHROPE;
